...
#SBATCH --job-name=retrieve_bm25_decomposer_openai
...
export WORKSPACE=$SSD_MOUNT/benchmark/retrieve_decomposer_openai
mkdir -p $WORKSPACE/bm25_title $WORKSPACE/bm25_abstract $WORKSPACE/bm25_text

cd benchmark

# Prepare the queries.
python3 -m sources.retrieve_decomposer_openai \
